...
function sayHelloBackground(){
    sayHelloButton.classList.remove('say-hello-original');
    sayHelloButton.classList.add('say-hello-background');
}
...
function sayHelloOriginal(){
    sayHelloButton.classList.remove('say-hello-background');
    sayHelloButton.classList.add('say-hello-original');
}

sayHelloButton.addEventListener('mouseout', sayHelloOriginal);
...
const repeatMyParam = () => {
    axios.get(`http://localhost:3000/repeat/${'Veggie Chips'}`)
        .then(function(res){
            document.getElementById('repeat-button').textContent = res.data;
            console.log(res.data);
...
// CODE HERE
function getRequest(){
    axios.get(`http://localhost:3000/query-test/?myQuery1=banana&?myQuery2=apple`)
        .then(function(res){
            console.log(res.data);
        })
}
...
//If I send more than one query, I get a response saying that I sent more than one query. If I don't send any then the response
//tells me that I didn't send any.
